...
language plpgsql as $$
begin
  update smartlocks.lock_commands as command_row set status='Expired',details='Command expired before delivery' where command_row.status='Queued' and command_row.expires_at_utc <= now();
...
begin
  update smartlocks.lock_commands as command_row set status='Published',published_at_utc=now() where command_row.id=p_id and command_row.status='Queued' and command_row.expires_at_utc>now();
  return found;
end; $$;
...
begin
  ...
  update smartlocks.lock_commands as command_row set status=case when p_outcome='Unlocked' then 'Acknowledged' else 'Failed' end,acknowledged_at_utc=p_occurred_at_utc,outcome=p_outcome,acknowledgement_nonce=btrim(p_nonce),details=nullif(btrim(p_details),'') where command_row.id=p_id and command_row.device_id=p_device_id and command_row.status in ('Queued','Published') and command_row.expires_at_utc>=p_occurred_at_utc and command_row.acknowledgement_nonce is null;
  return found;
end; $$;
